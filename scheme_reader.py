...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            src.remove_front()  # Removing '.'
            expression = scheme_read(src)  # Reading rest of expression
            if src.current() != ')':
                raise SyntaxError("Too many expressions after '.'")
            src.remove_front()  # Removing ')'
            return expression
        else:
            next_exp = scheme_read(src)
            rest_comb = read_tail(src)
            return Pair(next_exp, rest_comb)
    except EOFError:
        raise SyntaxError('unexpected end of file')
